#!/usr/bin/env python3
"""
FSDP (NO Quantization) Fine-tuning Script for LLaMA-2-7B on Dolly-15K
This script runs FSDP in full bfloat16 precision.
...
def main():
    parser = argparse.ArgumentParser(description="FSDP bf16 Fine-tuning")
    # Model/Data
    parser.add_argument("--model_id", type=str, default="meta-llama/Llama-2-7b-hf")
    parser.add_argument("--dataset_name", type=str, default="databricks/databricks-dolly-15k")
    parser.add_argument("--output_dir", type=str, default="./results/dolly_lora_llama2_7b_fsdp_bf16")
    # Training
    ...
    try:
        ...
        # FSDP requires loading the model on the CPU (device_map=None)
        # Load in bfloat16 instead of 4-bit
        ...
        # Explicitly disable KV cache
        model.config.use_cache = False

        if rank == 0:
            logger.info(f"✓ Model {args.model_id} loaded in bf16 on Rank 0")
            logger.info(f"Model use_cache set to: {model.config.use_cache}")
        ...
        if rank == 0:
            logger.info("Model loaded. SFTTrainer will handle PEFT/FSDP wrapping.")
        ...
        if rank == 0:
            logger.info("\nStarting FSDP (bf16) fine-tuning...")
        ...
        logger.info(f"Rank {rank} finished training. Waiting at barrier...")
        ...
        if rank == 0:
            ...
            if train_logs and eval_logs:
                ...
                fig.suptitle('Training & Validation Summary (FSDP, Rank 0)', fontsize=16, y=1.02)
